...
$sqln = "SELECT realip.ip_address,blocked.status FROM realip JOIN blocked ON realip.realip_id = blocked.realip_id";
$stmtn = $pdo->query($sqln);
...
if($newip === $row['ip_address'] && $row['status']==1){
  die("connection has been dropped");

}
if($newip === $row['ip_address'] && $row['status']==0){
  require_once('form_get.php');
}
